#!/usr/bin/env python3
"""
Generate a list of all removed words from the word frequency analysis.
This script tracks words removed in Step 2 (after merging) and Step 3 (after stemming).
"""
import json
import re
...
def main():
    ...
    output_file = base_dir / 'word_frequency_analysis_removed_words.txt'
    ...
    # Step 2: Merge word variants and remove specified words
    print("\nStep 2: Merging word variants and removing specified words...")
    
    # First, merge variants
    merged_freq = Counter()
    for word, freq in sorted_raw:
        merged = merge_word(word)
        merged_freq[merged] += freq
    
    # Then, remove specified words
    removed_words_step2 = []  # Track removed words with frequencies
    
    for word, freq in merged_freq.items():
        word_lower = word.lower().strip()
        if word_lower in WORDS_TO_REMOVE:
            removed_words_step2.append((word, freq))
    
    # Sort removed words by frequency
    removed_words_step2_sorted = sorted(removed_words_step2, key=lambda x: x[1], reverse=True)
    total_removed_freq_step2 = sum(freq for _, freq in removed_words_step2_sorted)
    
    print(f"  Removed: {len(removed_words_step2):,} unique words")
    print(f"  Total frequency removed: {total_removed_freq_step2:,}")
    
    # Step 3: Track words removed after stemming
    print("\nStep 3: Checking words removed after stemming...")
    
    # Get words that passed Step 2
    filtered_words = []
    for word, freq in merged_freq.items():
        word_lower = word.lower().strip()
    ...
    removed_words_step3 = []  # Track removed words with frequencies
    
    for word, freq in filtered_words:
        stemmed = simple_stem(word)
        stemmed_lower = stemmed.lower().strip()
        
        # Apply merge again after stemming (in case stemming created a variant)
        merged_stemmed = merge_word(stemmed_lower)
        
        # Check if merged/stemmed word is in removal list
        if merged_stemmed.lower().strip() in WORDS_TO_REMOVE:
            removed_words_step3.append((word, merged_stemmed, freq))
    
    # Sort removed words by frequency
    removed_words_step3_sorted = sorted(removed_words_step3, key=lambda x: x[2], reverse=True)
    total_removed_freq_step3 = sum(freq for _, _, freq in removed_words_step3_sorted)
    
    print(f"  Additional words removed in Step 3: {len(removed_words_step3):,} unique words")
    print(f"  Total frequency removed: {total_removed_freq_step3:,}")
    
    # Write removed words file
    print(f"\nWriting removed words list: {output_file}")
    removed_lines = []
    removed_lines.append("=" * 80)
    removed_lines.append("REMOVED WORDS - COMPLETE LIST")
    removed_lines.append("=" * 80)
    removed_lines.append("")
    
    # Step 2 removed words
    removed_lines.append("STEP 2: WORDS REMOVED AFTER MERGING")
    removed_lines.append("-" * 80)
    removed_lines.append(f"Total removed: {len(removed_words_step2_sorted):,} unique words")
    removed_lines.append(f"Total frequency removed: {total_removed_freq_step2:,}")
    removed_lines.append("")
    removed_lines.append(f"{'Rank':<8} {'Word':<40} {'Frequency':<12}")
    removed_lines.append("-" * 80)
    
    for rank, (word, freq) in enumerate(removed_words_step2_sorted, 1):
        removed_lines.append(f"{rank:<8} {word:<40} {freq:<12}")
    
    removed_lines.append("")
    removed_lines.append("")
    
    # Step 3 removed words
    removed_lines.append("STEP 3: WORDS REMOVED AFTER STEMMING")
    removed_lines.append("-" * 80)
    removed_lines.append(f"Total removed: {len(removed_words_step3_sorted):,} unique words")
    removed_lines.append(f"Total frequency removed: {total_removed_freq_step3:,}")
    removed_lines.append("")
    removed_lines.append(f"{'Rank':<8} {'Original Word':<30} {'Stemmed Word':<30} {'Frequency':<12}")
    removed_lines.append("-" * 80)
    
    for rank, (original, stemmed, freq) in enumerate(removed_words_step3_sorted, 1):
        removed_lines.append(f"{rank:<8} {original:<30} {stemmed:<30} {freq:<12}")
    
    removed_lines.append("")
    removed_lines.append("")
    removed_lines.append("=" * 80)
    removed_lines.append("SUMMARY")
    removed_lines.append("=" * 80)
    removed_lines.append(f"Total unique words removed in Step 2: {len(removed_words_step2_sorted):,}")
    removed_lines.append(f"Total unique words removed in Step 3: {len(removed_words_step3_sorted):,}")
    removed_lines.append(f"Total frequency removed in Step 2: {total_removed_freq_step2:,}")
    removed_lines.append(f"Total frequency removed in Step 3: {total_removed_freq_step3:,}")
    removed_lines.append(f"Grand total frequency removed: {total_removed_freq_step2 + total_removed_freq_step3:,}")
    ...
    print("SUMMARY")
    print("=" * 80)
    print(f"Total unique words removed in Step 2: {len(removed_words_step2_sorted):,}")
    print(f"Total unique words removed in Step 3: {len(removed_words_step3_sorted):,}")
    print(f"Total frequency removed in Step 2: {total_removed_freq_step2:,}")
    print(f"Total frequency removed in Step 3: {total_removed_freq_step3:,}")
    print(f"Grand total frequency removed: {total_removed_freq_step2 + total_removed_freq_step3:,}")
    print(f"\nOutput file: {output_file}")
    print("\n✓ Removed words file generated successfully!")
